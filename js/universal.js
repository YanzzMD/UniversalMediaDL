...
(function App() {
  ...
  // ─── TIKTOK VALIDATION ─────────────────────────────────────────
  const IG_PATTERNS =[ /^https?:\/\/(www\.)\.com\/.+/ ];
  
  function isValidUrl(url) { 
    return /^https?:\/\//.test(url.trim()); // Menerima SEMUA URL HTTP/HTTPS
  }
  ...
  function validateUrl(url) {
    ...
    if (isValidUrl(url)) {
      ...
      urlValidation.textContent = '✗ Hanya mendukung URL Valid di halaman ini';
